...
\begin{theorem}[Carathéodory] \label{thm:ExtensionToBoundary}
    The conformal mapping $\Phi:D\to G$ can be extended to a continuous mapping $\Phi:\bar{D}\to\bar{G}$ if the boundary $\Gamma$ of $G$ consists of a closed curve. \cite{Wegmann2005}, p. 357
\end{theorem}
\begin{definition}
    \red{Ahlfors map}
\end{definition}
...
\red{SOMEWHERE WE NEED TO ADD THE RESTRICTION OF $\Phi(0)=0, \quad\Phi'(0)>0$ FOR LATER REFERENCE} \label{InitialConditionsOnPhi}
...
\end{definition}
This is a Hilbert space over $\reals$. The subspaces of real functions are denoted $L_{\reals}^2$ and $W_{\reals}$ respectively. Note that we can decompose $W$ into the direct sum of the subspaces $W = W^{+} \oplus W^{-}$ where $f \in L^2$ is decomposed as follows into its Fourier series:
...
$$
Recall the definition of $\Phi$ as the mapping from the boundary of the disk \red{VIA ETA} and subject to \ref{InitialConditionsOnPhi}. This can now be expressed as $\Phi \in W^{+}$\red{HUH}.
Then, since the boundary function \ref{boundaryCorrespondence} maps the unit circle to $\Gamma$ there exists \red{why} a function $\hat{u}$ such that 
$$\Phi(t)=\eta(t+\hat{u}(t)) \quad \forall t.$$
By the implicit function theorem \ref{thm:ImplicitMappingTheorem} $\hat{u}$ is continuously differentiable, hence $\hat{u} \in W_{\reals}$.
This tells us that the function $\Phi$ we are looking for lies in the intersection of a certain manifold $M:= \{u \in W_{\reals}: \eta(t+u(t))\}$ with our space $W^{+}$. This formulation is used in chapter \ref{chap:AlternatingProjections}.
...
\end{cases}
$$ 
In particular, $x*=y*$ if $P\cap Q \neq \emptyset$. \cite{Braun2023AltProjections}
