...
We are given a simply connected bounded domain $\Omega\subset\C$ with boundary $\Gamma$. The task is to find an angle-preserving mapping $\psi$ from the unit disk $\disk$ to $\Omega$. 
We are also given a curvilinear triangular mesh $\widehat{\mathcal{M}}$ on $\disk$, and a $2\pi$-periodic parametrization $$\eta: [0, 2\pi]^2 \to \Gamma \quad \text{ with } \quad \eta \in C^0 ([0,2\pi])^2$$ by its fourier series representation.
\red{DANN WÄRE $\eta$ HIER AUCH NUR EINDIMENSIONAL, WENN WIR $\C$ BENUTZEN STATT $\R^2$?}
...
\end{equation}
\red{\cite{Kythe1998_symmsintegralequation}}
