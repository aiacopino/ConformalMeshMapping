...
Finally, multiplying $f$ with a suitable unimodular complex number gives the desired conformal map from $\Omega$ to $\mathbb{D}$ with $$f(0) = 0 \text{ and } f'(0) > 0.$$ \label{InitialConditionsOnPhi} $\hfill\qed$
...
\red{finding $\psi$ means solving Laplace's equation}
In Riemann's thesis, conformal mappings are a mere special case of the more general family of boundary value problems of analytic functions on the unit disk $\disk$ or in its exterior $\disk^{-}:=\{z: |z|>1\}$. The simplest such problem is called \textbf{Dirichlet problem} and can be described as finding $\psi$ analytic in $\disk$, continuous in $\bar{\disk}$ and satisfying $$Re(\psi(e^{it}))=\psi(t)$$ on the boundary of $\disk$, where $\psi$ is $2\pi$-periodic and Hölder continuous.
...
Thus, one way to construct conformal maps is to solve Laplace's equation with suitable boundary conditions. This formulation of the problem is called Riemann-Hilbert problem, and it is a special kind of boundary value problem.
