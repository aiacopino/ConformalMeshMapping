...
\red{EHAT IS THE DIFF BETW ALT PROJ AND CONJ FUNC}
\red{ALSO NEUMANN KERNEL AND CONJ FUNC ARE NOT IN COMPTABLE}
...
Furthermore, discretizing the operator $K$ (\ref{operatorK_N}) using Wittich's method yields the operator $K_N$ which can be computed efficiently using FFT.
...
Hence, Zipper, OAP and Wegmann's methods seem suitable for our problem, but Wegmann's Method is more accurate with faster convergence while OAP is easier to implement.
