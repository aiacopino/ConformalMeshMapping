...
The above equation (\ref{eq:h_k+1}) is a non-homogeneous Riemann-Hilbert Problem \cite{AnosovBolibruch1994_riemannhilbertproblems} of two unknowns, $U_k$ and $\psi_{k+1}$, which is solved using a method described in \cite[§ 87]{Muskhelishvili1977} and \cite{Wegmann1978_newtonverfahren}.
...
This corresponds to the geometric observation that we are essentially moving the current analytic approximation $\psi_{k+1}$ towards the boundary along the tangent, i.e.
\begin{equation}
    \psi_{k+1}-\eta((S_k(\zeta))) = c \cdot \dot\eta(S_k(\zeta))
\end{equation}
where $c\in\R$. Hence, 
\begin{equation} \label{eq:imaginaryParts}
    \text{Im}(\frac{\psi_{k+1}-\eta(S_k(\zeta))}{\dot\eta(S_k(\zeta))})=0 \iff
    \text{Im}(\frac{\psi_{k+1}}{\dot\eta(S_k(\zeta))})= \text{Im}(\frac{\eta(S_k(\zeta))}{\dot\eta(S_k(\zeta))}).        
\end{equation}
Note that since for a complex number Im$(a+ib) = b = -i\text{Re}(a+ib)$, we have
\begin{equation}
    \text{Im}(\frac{\psi_{k+1}}{\dot\eta(S_k(\zeta))}) = -i\text{Re}(\frac{\psi_{k+1}}{\dot\eta(S_k(\zeta))}) = \text{Re}(\frac{\psi_{k+1}}{i\dot\eta(S_k(\zeta))}),
\end{equation}
which can be plugged into equation (\ref{eq:imaginaryParts}) to yield exactly equation (\ref{eq:RHproblem}).
This eliminates $U_k$ from the equation and yields a Dirichlet problem for $\psi_{k+1}$. The analytic $\psi_{k+1}$ can then be reconstructed by adding the imaginary part \cite[Lemma 14.2]{Henrici1986_vol3}using the operator $K$ defined in chapter \ref{operatorK_N}. 
...
Note that the condition (\ref{eq:InitialConditionsOnPsi}) could cause the algorithm to break or diverge if the target region does not contain the origin. 
This can be resolved by first translating the region such that the condition $\psi(0)=0$ is satisfied, and then adding the shift back after computation of the conformal map. 
...
\red{TODO: ausprobieren mit quadrat (truncate infinite fourier series)
}
...
add images of mesh map to documentation!!!

\red{TO DO: CHECK METHOD 1 IMPLEMENTATION WHY IT DONT CONVERGE}
\red{TO DO: TEST INVERTED ELLIPSEAND COMPARE TO CONFORMAL MAP}
\red{TO DO: RUN EXPERIMENTS AGAIN, COMPUTE DISTAANCE OF COMPUTED MAP TO THE ACTUAL BOUNDARY TO MEASURE EXACTNESS OF SOLUTION}
\red{TO DO: FIND INITIAL GUESS FOR KITE}
maybe test star with more edges?
...
Some experiments with the solver have \red{confirmed} the results Wegmann got: Verfahren 2 typically converges faster, while Verfahren 1 typically yields more accurate results. 
We present some of the results in this chapter. 
Gaier provides some ineresting examples of known conformal maps to compare the solver to, which yields a direct measure of the exactness of the computed conformal map.
The measure of exactness in the below tables is calculated as the norm of the difference of the functions.

%%%%%%%%%%%% ECCENTRIC CIRCLE %%%%%%%%%%%%%%%%%%%
\begin{figure}
    \includegraphics[width=0.5\textwidth]{img/eccentric_v1_rel.2.png}
    \includegraphics[width=0.5\textwidth]{img/eccentric_v2_rel.2.png}
...
\begin{table}
    ...
    \begin{tabular}{|p{0.2\textwidth}|p{0.35\textwidth}|p{0.35\textwidth}|}
        ...
        Verfahren & 1 & 2 \\
        \hline
        Exactness & 53.84189489305918 & 54.26115719239023 \\
        \hline
        Iteration \# & \multicolumn{2}{c}{Norm of correction/ update step $\lVert U_{k} \rVert$} \\
        0 & 151.98901475588582 & 151.98901475588582 \\
        1 & 114.21594140252218 & 194.00835541321004 \\
        2 & 89.44329058620063 & 160.87394441489494 \\
        3 & 72.09916724327302 & 121.58970214106796 \\
        4 & 59.35011962259898 & 90.8824454960736 \\
        5 & 49.618271141773796 & 68.44494057994135 \\
        6 & 41.96440935321939 & 52.09880562369982 \\
        7 & 35.80001091040178 & 40.049386024748614 \\
        8 & 30.740615820537126 & 31.041704607068777 \\
        9 & 26.525954880686037 & 24.220616542709806 \\
        10 & 22.974078804192068 & 18.99867133961369 \\
        \dots & \dots & \dots \\
        75 & 0.009990678563315262 & 9.93750931682235e-06 \\
        \dots & \dots &  \\
        150 & 9.93750931682235e-06 & \\
...
\begin{figure}
    \includegraphics[width=0.6\textwidth]{img/invellipse_v1_p.6_rel.4.png}
    \includegraphics[width=0.6\textwidth]{img/invellipse_v2_p.6_rel.4.png}
    \caption{Gaier's inverted ellipse, which is also Wegmann's flagship example from the 1978 paper. \cite[Anhang 5, p. 264]{Gaier1964}}
\end{figure}
\begin{table}
    ...
    \begin{tabular}{|p{0.2\textwidth}|p{0.35\textwidth}|p{0.35\textwidth}|}
        ...
        Verfahren & 1 & 2 \\
        \hline
        Exactness & 1.3977742222951416e-12 & 1.3975176541099112e-12 \\
        \hline
        Iteration \# & \multicolumn{2}{c}{Norm of correction/ update step $\lVert U_{k} \rVert$} \\
        ...
        \dots & \dots & \dots \\
        55 & 6.050613540055119e-11 & 6.056667137459197e-11 \\
        \hline
    \end{tabular}
\end{table}
...
\centering
\begin{figure}
    \includegraphics[width=0.6\textwidth]{img/invellip_v1_p.4.png}
    \includegraphics[width=0.6\textwidth]{img/invellip_v2_p.4.png}    
    \includegraphics[width=0.6\textwidth]{img/invellip_v1_p.4_detail.png}
    \includegraphics[width=0.6\textwidth]{img/invellip_v2_p.4_detail.png}
    \caption{Gaier's inverted ellipse ut more convex. \cite[Anhang 5, p. 264]{Gaier1964}}
\end{figure}
\begin{table}
    \caption{Inverted ellipse discretized at $N = 512$ points, with convexity parameter $p = 0.4$. We have tried with even more convex, but already $p=0.3$ is quite a challenge, relaxation must be chosen extremely small and many more discretisation points (at least 1024, and even then only an accuracy of $10^-4$ can be attained.)}
    \centering
    \begin{tabular}{|p{0.2\textwidth}|p{0.35\textwidth}|p{0.35\textwidth}|}
        \hline
        Verfahren & 1 & 2 \\
        \hline
        Exactness & 1.2988876338244771e-11 & 0.0001315917931317448 \\
        \hline
        Iteration \# & \multicolumn{2}{c}{Norm of correction/ update step $\lVert U_{k} \rVert$} \\
        0 & 36.577695800268735 & 36.577695800268735 \\
        1 & 23.998212599782647 & 23.99821265406515 \\
        2 & 15.288233808871608 & 15.288233845161551 \\
        3 & 9.530324234394648 & 9.530324253913129 \\
        4 & 5.854994260623575 & 5.854994270199947 \\
        5 & 3.563899762601892 & 3.563899766424457 \\
        6 & 2.1569923565730837 & 2.1569923572638827 \\
        7 & 1.300975206997786 & 1.3009752061064879 \\
        8 & 0.7830389289227928 & 0.7830389273436897 \\
        9 & 0.47070941447541376 & 0.4707094127129884 \\
        10 & 0.2827451906950015 & 0.2827451890177375 \\
        \dots & \dots & \dots \\
        53 &  8.177427768306776e-11 & 8.178251083046234e-11 \\
        \hline
    \end{tabular}
\end{table}
\centering
...
\begin{figure}
    \caption{$L^2$ Distance between computed conformal map and known conformal map on inverted ellipse, $p = 0.3$: \\
    Verfahren 1: $3.710212908531002e-06$, Verfahren 2: $3.7636081451245516e-06$}
    \includegraphics[width=0.6\textwidth]{img/invellip_v1_p.3.png}
    \includegraphics[width=0.6\textwidth]{img/invellip_v2_p.3.png}
    \includegraphics[width=0.6\textwidth]{img/invellip_v1_p.3_detail.png}
    \includegraphics[width=0.6\textwidth]{img/invellip_v2_p.3_detail.png}
\end{figure}
