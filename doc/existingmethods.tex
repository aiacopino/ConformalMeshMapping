...
\begin{definition}
    A \textbf{Polygon $P$} is a piecewise linear Jordan curve with finitely many line segments connecting corner points $z_1, ..., z_N$.
\end{definition} 
...
Note also that $P$ need not be bounded, since we can add vertices at complex infinity with exterior angles ranging from $[1,3]$. 
These angles are defined to be $2\pi$ minus the external angle formed in the plane by the two lines meeting at infinity \cite[Fig. 1 and 2]{Trefethen1980_SCnumericalcomputation}. 
...
\begin{equation} % TREFETHEN W = MY Z, TREFETHEN Z = MY V
    z=\psi(v)= z_c + C\int_{0}^{v} \prod_{k=1}^{N} (1-\frac{\zeta}{v_k})^{-\theta_k} d\zeta \text{ for } v\in\disk
\end{equation}

Note that $1-\frac{\zeta}{v_k} \in \{|z-1|<1 \}$ for $|v|<1$. Hence we can define a branch of logarithm with branch cut along the negative real axis and define
\begin{equation}
    (1-\frac{\zeta}{v_k})^{-\theta_k} = \exp(-\theta_k \log(1-\frac{\zeta}{v_k}))
\end{equation} 
such that $\psi(v)$ defines an analytic function on $\disk$ which is continuous except at the vertices $v_k$ \cite[Eq. 1.2]{Trefethen1980_SCnumericalcomputation}.
The formula is constructed such that the angles at the vertices correspond precisely to the exterior angles we need, hence the image of $\psi$ is a polygon with the correct angles. However, the lengths of the line segments need not be correct: This is where the choice of parameters $v_1, ..., v_N, z_c$ and $C$ comes in, and this is where the computational challenge in this method lies.
...
Next, the scaling factor $C$ is defined by the formula \cite[Eq. 2.3]{Trefethen1980_SCnumericalcomputation}
...
\cite[Fig. 3]{Trefethen1980_SCnumericalcomputation}
...
The computed map is only approximately conformal, and is obtained as a composition of conformal maps onto slit halfplanes. 
There are different versions of this algorithm adapted to the shape of the slits.
...
\begin{definition}[{\cite[page 31]{ahlfors1979_complexanalysis}, \cite[page 209]{SteinShakarchi2003_complexanalysis}}]
    A \textbf{Möbius transformation} is a function on the extended complex plane $\hat\C:=\C\cup\{\infty\}$ which is uniquely determined by where it sends three points. It has the form
    $$ f(z) = \frac{az + b}{cz + d} $$
...
\cite[Fig. 2]{marshall2006_convergencezipperalgorithmconformal}
\input{zipper_fa.tex}
...
\end{flushleft}
\cite[Fig. 3]{marshall2006_convergencezipperalgorithmconformal}
...
For $\Omega$ star-shaped with regard to the origin, Theodorsen's method \cite[page 64]{Gaier1964} can be used to find the conformal mapping $\psi:\disk\to\Omega$ satisfying the constraints (\ref{eq:InitialConditionsOnPsi}).
Since $\Omega$ is star-shaped, the boundary $\Gamma$ can be parametrised by the polar angle. 
Consequently, the boundary correspondence function $S(\varphi)$ defined in (\ref{eq:boundaryCorrespondence}) becomes the angular function $\theta(\varphi)$. 
Theodorsen's integral equation is derived as follows:
Note points on the unit circle are parametrised by $e^{i\varphi}$ while $\Gamma$ can be parametrised by $\rho(\theta)e^{i\theta}$ for $\varphi, \theta\in[0,2\pi]$ and $\rho(\theta)>0$ the radius function of $\Gamma$ (by star-shapedness of $\Omega$). 
Thus, the problem reduces to relating $\varphi$ and $\theta$ via a function $\theta(\varphi)$.
...
\begin{equation}\label{eq:helperfunctiontheodorsen}
    \begin{matrix*}[r]
        ...
        \log(\frac{\rho(\theta)e^{i\theta}}{e^{i\varphi}}) = \\
        \log(\rho(\theta)e^{i(\theta-\varphi)}) = \\
        \log(\rho(\theta))+i(\theta - \varphi)
    \end{matrix*}
\end{equation}
where the second row follows from $z = e^{i\varphi}$.
...
Unlike Fredholm integral equations of the second kind like Theodorsen's equation, where the singularity of the kernel creates numerical instabilities, Symm's equation is easily solvable by numerical methods \cite[Chapter 9.1]{Kythe1998_symmsintegralequation}, \cite[Chapter 3.1]{Wegmann2005_num_methods_confmapping}. One of these is Amano's method.
...
\begin{definition}[{Sobolev Space, \cite[Definition 3.2]{AdamsFournier2003_sobolev}}]
    The \textbf{Sobolev space $W^{m,p}$} is defined as the space of all absolutely continuous functions $f\in L^p$ such that the derivative $D^pf$ exists and is also in $L^p$. The inner product on $W^{m,p}$ is defined as
    $$(f,g)_{W^{m,p}} = \left (\sum_{0 \leq |k| < m} \lVert D^kf\rVert_{L^p}^p \right )^{1/p}.$$
\end{definition}
...
Conformal mappings can be considered as a special case of a more general class of boundary value problems called nonlinear \textbf{Riemann-Hilbert problems} and can be solved by Newton iteration over linear RH problems \cite[Chapter 2.5]{Wegmann2005_num_methods_confmapping}.
%The projection onto the tangent space described above is analytically equivalent to a linearisation of the boundary value problem. Wegmann \cite{Wegmann1978_newtonverfahren} showed that the update shift $U_k(t)$ is the solution to the linearised equation below.
...
(Note that the problem boils down to finding the correct boundary correspondence function $S$ and is nonlinear in this unknown.)
Assume that the approximation for $\psi$ after $k$ steps be given by $\psi_k(\zeta)=\psi_k(e^{i\theta})=\eta(S_k(\theta))$. 
$$
...
\end{tikzpicture}
$$ 
We improve the approximation by moving the points $$\{\eta(S_k(\theta))\mid \theta = \arg(\zeta)\in[0,2\pi], \zeta\in\del\disk \}$$ along their tangents towards the boundary curve, i.e. by finding a correction function $U_k:[0,2\pi]\to[0,2\pi]$ such that \cite[Equation 30]{Wegmann1978_newtonverfahren}
\begin{equation}\label{eq:h_k+1}
    \eta(S_k(\theta))+U_k(\theta)\dot\eta(S_k(\theta)) = \psi_{k+1}(\zeta)
\end{equation}
...
The above equation (\ref{eq:h_k+1}) is a non-homogeneous and non-linear Riemann-Hilbert Problem \cite{AnosovBolibruch1994_riemannhilbertproblems} of two unknowns, $U_k$ and $\psi_{k+1}$, which is solved using a method described in \cite[§ 87]{Muskhelishvili1977} and \cite{Wegmann1978_newtonverfahren} (see section \ref{chap:implementation}). 
This type of problem of finding an analytic function for a given relation of the limiting values of its real and imaginary parts of some was first studied by Riemann and the specific problem here was introduced by Hilbert, hence the name \cite[§89]{Muskhelishvili1977}.

By the Riemann Mapping Theorem, it admits a unique solution only if we fix three degrees of freedom. 
The first two are fixed by requiring that a specific boundary point $z_0$ corresponds to the target parameter $s_0$ (where $\eta(s_0) = c_0$). 
A direct coordinate constraint $\psi_{k+1}(e^{iz_0}) = c_0$ is geometrically impossible because $\psi_{k+1}(e^{iz_0})$ is constrained to the tangent vectornot typically lying on the boundary $\Gamma$, while $c_0$ lies on the curve $\Gamma$. 
Therefore, the normalisation condition is transferred to the parameter space:
We force the updated parameter function to satisfy $S_{k+1}(z_0) = s_0$, which directly yields the necessary boundary condition for $U_k$ \cite[Equation 2.4a]{Wegmann1978_newtonverfahren}:
\begin{equation}
    U_k(z_0)=s_0-S_k(z_0) \in \R.
\end{equation}

This corresponds to the geometric observation that we are moving the current analytic approximation $\psi_{k+1}$ towards the boundary along the tangent, i.e.
\begin{equation}
    \psi_{k+1}(\zeta)-\eta((S_k(\theta))) = U_k \cdot \dot\eta(S_k(\theta))
\end{equation}
where $U_k$ is real-valued.
To find $U_k$, equation (\ref{eq:h_k+1}) is rewritten into \cite[Equation 3.4]{Wegmann1978_newtonverfahren}:
\begin{equation}
    U_k = \frac{\psi_{k+1}}{\dot\eta(S_k)} - \frac{\eta(S_k)}{\dot\eta(S_k)} \iff
\end{equation}
\begin{equation}
    \text{Im}(\frac{\psi_{k+1}(\zeta)-\eta(S_k(\theta))}{\dot\eta(S_k(\theta))})=0 \iff
\end{equation}
\begin{equation} \label{eq:imaginaryParts}
    \text{Im}(\frac{\psi_{k+1}(\zeta)}{\dot\eta(S_k(\theta))})= \text{Im}(\frac{\eta(S_k(\theta))}{\dot\eta(S_k(\theta))}).        
\end{equation}
Note that since for a complex number Im$(a+ib) = b = \text{Re}(-i(a+ib))$, we have
\begin{equation}
    \text{Im}(\frac{\psi_{k+1}(\zeta)}{\dot\eta(S_k(\theta))}) = \text{Re}(-i\frac{\psi_{k+1}(\zeta)}{\dot\eta(S_k(\theta))}) = \text{Re}(\frac{\psi_{k+1}(\zeta)}{i\dot\eta(S_k(\theta))}),
\end{equation}
which can be plugged into equation \eqref{eq:imaginaryParts} to yield the linear Riemann-Hilbert problem for $\psi_{k+1}$:
\begin{equation}\label{eq:RHproblem}
    \text{Re}(\frac{\psi_{k+1}(\zeta)}{i\dot\eta(S_k(\theta))})= \text{Im}(\frac{\eta(S_k(\theta))}{\dot\eta(S_k(\theta))}).
\end{equation}
The full analytic $\psi_{k+1}$ can then be reconstructed by adding the imaginary part \cite[Lemma 14.2]{Henrici1986_vol3} using the (Hilbert transform) operator $K$ defined in chapter \ref{operatorK_N}. 
Next, $U_k$ is computed by plugging $\psi_{k+1}$ into the equation \eqref{eq:h_k+1} and taking the real part.
Finally, we get an approximation for the parametrization $S$ by 
\begin{equation}
    S_{k+1}(\theta):= S_k(\theta)+U_k(\theta) \text{ for } \theta \in [0, 2\pi].
\end{equation}
\bigskip
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \STATE \textbf{Initialise:} $S_0(\theta) = \arg(\zeta) = \arg(e^{i\theta})$ (Identity) or guess satisfying $S_0(z_0)=s_0$.
        \WHILE {not converged}
            \STATE \textit{// 1. Evaluate current boundary geometry}
            \STATE $f_k(\theta) := \eta(S_k(\theta))$ \hfill // Position on boundary
            \STATE $g_k(\theta) := \dot\eta(S_k(\theta))$ \hfill // Tangent vector on boundary
            
            \STATE \textit{// 2. Solve Riemann-Hilbert Problem with anchoring}
            \STATE Find analytic function $\psi_{k+1}$ on $\mathbb{D}$ satisfying:
            \begin{equation*}
                \text{Re}\left(\frac{\psi_{k+1}(\zeta)}{i g_k(\theta)}\right) = \text{Im}\left(\frac{f_k(\theta)}{g_k(\theta)}\right) \quad \forall \zeta \in \partial\mathbb{D}
            \end{equation*}
            \STATE Fix degrees of freedom by anchoring $\psi_{k+1}(z_0)=\eta(S_k(s_0))$.

            \STATE \textit{// 3. Compute correction and update}
            \STATE $U_k(\theta) := \text{Re}\left( \frac{\psi_{k+1}(\zeta) - f_k(\theta)}{g_k(\theta)} \right)$
            \STATE $S_{k+1}(\theta) := S_k(\theta) + U_k(\theta)$
        \ENDWHILE
    \end{algorithmic}
\end{algorithm}
...
The Zipper method is very well implementable and robust (it can handle rough boundaries including fractals), but there is already a MATLAB implementation (zippit).
