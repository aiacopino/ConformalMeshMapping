...
\end{cases}$$ 
\red{ADD IMAGE IF PATIENCE REMAINS}
...
For $\Omega$ star-shaped with regard to the origin (wlog; otherwise we can precompose with a preliminary map), Theodorsen's method \cite[page 64]{Gaier1964} can be used to find the conformal mapping $\psi:\disk\to\Omega$ satisfying the initial conditions (\ref{eq:InitialConditionsOnPsi}).
...
The discretization as in \cite{Gutknecht1983_theodorsen_equation_FFT} is done by naming $Y := \theta - \varphi$ the conjugate function of $X:= \log(\rho(\theta))$.
...
\end{equation} 
for $s:=(\frac{k\pi}{N})_{k\in[2N-1]}$ where $y$ approximates $Y(s)$ and $\eta$ is the boundary correspondence function (\ref{eq:boundaryCorrespondence}).
...
It also theoretically needs $\Omega$ to be star-shaped in order to converge \cite{Wegmann1978_newtonverfahren}, but has the advantage of being an easily computable fixed point equation in this case. 
