...
\end{tikzpicture}
$$ 
We improve the approximation by moving the points $\{\eta(S_k(\zeta))\mid \zeta\in\del\disk\}$ along their tangents towards the boundary curve, i.e. by finding a correction function $U_k:\del\disk\to[0,2\pi]$ such that \cite[Equation 30]{Wegmann1978_newtonverfahren}
...
A direct coordinate constraint $\psi_{k+1}(z_0) = c_0$ is geometrically impossible because $\psi_{k+1}(z_0)$ is constrained to the tangent vectornot typically lying on the boundary $\Gamma$, while $c_0$ lies on the curve $\Gamma$. 
Therefore, the normalization condition is transferred to the parameter space:
...
\begin{equation}
    \psi_{k+1}(\zeta)-\eta((S_k(\zeta))) = c \cdot \dot\eta(S_k(\zeta))
\end{equation}
where $c\in\R$. 

To find $U_k$, equation (\ref{eq:h_k+1}) is rewritten into \cite[Equation 3.4]{Wegmann1978_newtonverfahren}:
\begin{equation}
    \R \ni U_k = \frac{\psi_{k+1}}{\dot\eta(S_k)} - \frac{\eta(S_k)}{\dot\eta(S_k)} \iff
\end{equation}
\begin{equation}
    \text{Im}(\frac{\psi_{k+1}(\zeta)-\eta(S_k(\zeta))}{\dot\eta(S_k(\zeta))})=0 \iff
\end{equation}
\begin{equation} \label{eq:imaginaryParts}
    \text{Im}(\frac{\psi_{k+1}(\zeta)}{\dot\eta(S_k(\zeta))})= \text{Im}(\frac{\eta(S_k(\zeta))}{\dot\eta(S_k(\zeta))}).        
\end{equation}
Note that since for a complex number Im$(a+ib) = b = \text{Re}(-i(a+ib))$, we have
\begin{equation}
    \text{Im}(\frac{\psi_{k+1}(\zeta)}{\dot\eta(S_k(\zeta))}) = \text{Re}(-i\frac{\psi_{k+1}(\zeta)}{\dot\eta(S_k(\zeta))}) = \text{Re}(\frac{\psi_{k+1}(\zeta)}{i\dot\eta(S_k(\zeta))}),
\end{equation}
...
\begin{equation}\label{eq:RHproblem}
    \text{Re}(\frac{\psi_{k+1}(\zeta)}{i\dot\eta(S_k(\zeta))})= \text{Im}(\frac{\eta(S_k(\zeta))}{\dot\eta(S_k(\zeta))}).
\end{equation}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \STATE \textbf{Initialise:} $S_0(\zeta) = \arg(\zeta)$ (Identity) or guess satisfying $S_0(z_0)=s_0$.
        \WHILE {not converged}
            \STATE \textit{// 1. Evaluate current boundary geometry}
            \STATE $f_k(\zeta) := \eta(S_k(\zeta))$ \hfill // Position on boundary
            \STATE $g_k(\zeta) := \dot\eta(S_k(\zeta))$ \hfill // Tangent vector on boundary
            ...
            \end{equation*}
            \STATE Find analytic $\psi_{k+1}$ by adding the imaginary part via Hilbert transform \ref{eq:operatorK}:
            \begin{equation*}
                 \psi_{k+1} := \text{Re}(\psi_{k+1}) + i K \text{Re}(\psi_{k+1})
            \end{equation*}
