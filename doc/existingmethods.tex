...
This can be performed on a discretized grid of points on the boundary. In that case, the output is a set of Fourier coefficients, which allows for efficient point evaluations of $\psi$ and $D\psi$ using FFT.
...
Recall $\eta$ the differentiable parametrization of $\del\Gamma$ and $\dot\eta(s)\neq 0 \text{ } \forall s$. 
...
On the other hand, the Alternating Projections Method uses the boundary parametrization in its Fourier series form. The output of the discretized OAP is an interpolating polynomial, allowing for efficient point evaluations of $\psi$ and $D\psi$.
...
Lastly, Wegmann's Method (\ref{chap:WegmannMethod}) uses the boundary parametrization  and its derivative in form of Fourier coefficients as well as the tangent angle as inputs. The output of the algorithm is an approximation by trigonometric interpolating polynomials at the grid points. This allows for efficient point evaluations of both $\psi$ and its derivative \cite{Wegmann1984_convergence_iterative_method}.
...
Note the computational costs of both of these methods are mainly determined by the FFTs and this parameter is dependent on the number of grid points.

Hence, both OAP and Wegmann's method seem suitable for our problem, but Wegmann's Method is more accurate with faster convergence while OAP is easier to implement.

\bigskip

\red{THEORY SECTION ON FFT???}
\red{put in each method's description what exactly it gets its restriction from, e.g. pinpoint the bottleneck}
