...
\subsection{Wegmann's Method (Alternating Projections)}\label{chap:WegmannMethod}
...
It was originally derived as a Newton method for a nonlinear integral equation \cite{Wegmann1978_newtonverfahren} but later re-interpreted geometrically as Alternating Projections onto tangent spaces \cite{Wegmann1989_alternating_projections}.
We first give the geometrical intuition and start by introducing the necessary function spaces.
...
\subsubsection{Analytic Realisation (Newton's Method)}
...
Note that since $\psi_{k+1}(z_0)$ lies on the tangent through $\eta(S_k(z_0))$ we cannot have $\psi_{k+1}(z_0)=c_0$ unless $\eta(S_k(z_0))=c_0$. Therefore we set
\begin{equation}
    U_k(z_0)=s_0-S_k(z_0).
\end{equation}
We get an approximation for the parametrization $S$ by $$S_{k+1}(\zeta):= S_k(\zeta)+U_k(\zeta)$$ which can be understood as "new guess equals last guess plus some correction".

%\input{newton_iter.tex}
...
Note that this is a Newton method and thus only guaranteed to converge locally due to the linearisation of $M$. Hence why a "good" (close enough) initial guess is vital for numerical stability.
...
Lastly, Wegmann's Method of Alternating Projections uses the boundary parametrization in its Fourier series form.
The output of the discretized OAP is an interpolating polynomial, allowing for efficient point evaluations of $\psi$ and $D\psi$ \cite{Wegmann1984_convergence_iterative_method}.
Convergence is quadratic for analytic boundaries (as is known for Newton methods) and superlinear ($\in\bigO(N^{1+\mu})$) for $\eta\in C^{2+\mu}$ but depends on a "good enough" initial guess for the boundary correspondence function (\ref{eq:boundaryCorrespondence}) \cite[page 292]{Wegmann1989_alternating_projections}, \cite{Wegmann1984_convergence_iterative_method}.
It is one of the simplest and most robust methods for conformal mapping. However, it is not very accurate for reasonably sized grids, and converges very slowly for finer meshes \cite[page 389]{Wegmann2005_num_methods_confmapping}.
...
Hence, both Zipper and Wegmann's methods seem suitable for our problem, but Wegmann's Method is more accurate with faster convergence while Zipper seems easier to implement.
