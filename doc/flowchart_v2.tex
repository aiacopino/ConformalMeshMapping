...
\node (theodorsen) [process, left=1cm of dec_star] {Theodorsen's\\Method (\ref{chap:TheodorsenMethod})};

% Down Branch: General Smooth Methods
\node (general_bie_label) [process, below=1cm of dec_star] {General Smooth Methods:};
    % List
    \node (amano) [process, below=0.1cm of general_bie_label] {Amano's Method (\ref{chap:AmanosMethod})};
    \node (AP) [process, below=0.1cm of amano] {Alternating Projections (\ref{chap:APMethod})};
    \node (wegmann) [process, below=0.1cm of AP] {Wegmann's Method (\ref{chap:WegmannMethod})};
...
\node (dec_poly) [decision, right=1.5cm of dec_smooth] {$\Omega$ a Polygon?};
    ...
    \node (schwarz) [process, right=1cm of dec_poly] {Schwarz-\\Christoffel\\(\ref{chap:SchwarzChristoffelMethod})};
    ...
    \node (zipper) [process, below=0.1cm of general_group] {Zipper Method (\ref{chap:ZipperMethod})};
    \node (conjugate) [process, below=0.1cm of zipper] {Conjugate Function \\Method (\ref{chap:ConjugateFunctionMethod})};
    \node (prob) [process, below=0.1cm of conjugate] {Probabilistic\\Method (\ref{chap:ProbabilisticMethod})};
...
\draw [thin, gray] (general_bie_label) -- (amano);
\draw [thin, gray] (amano) -- (AP);
\draw [thin, gray] (AP) -- (wegmann);

\draw [thin, gray] (general_group) -- (zipper);
\draw [thin, gray] (zipper) -- (conjugate);
